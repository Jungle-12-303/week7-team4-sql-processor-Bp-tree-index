 ...
 */
typedef enum {
    ID_INDEX_CONDITION_NONE,
    ID_INDEX_CONDITION_EQ,
    ID_INDEX_CONDITION_GT,
    ID_INDEX_CONDITION_GE,
    ID_INDEX_CONDITION_LT,
    ID_INDEX_CONDITION_LE,
    ID_INDEX_CONDITION_BETWEEN
} IdIndexConditionType;

typedef struct {
    IdIndexConditionType type;
    int value;
    int lower;
    int upper;
} IdIndexCondition;

static int extract_id_index_condition(ASTNode *where_node, IdIndexCondition *condition) {
    ASTNode *column_node;
    ...
    condition->type = ID_INDEX_CONDITION_NONE;
    ...
    if (extract_between_where_parts(where_node, &column_node, &between_node, &lower_node, &upper_node)) {
        ...
        if (lower_node->value_type != AST_VALUE_NUMBER || upper_node->value_type != AST_VALUE_NUMBER) {
            return 0;
        }
        if (!parse_integer_literal(lower_node->text, &condition->lower)) {
            return 0;
        }
        if (!parse_integer_literal(upper_node->text, &condition->upper)) {
            return 0;
        }

        condition->type = ID_INDEX_CONDITION_BETWEEN;
        return 1;
    }
    ...
    if (!parse_integer_literal(value_node->text, &condition->value)) {
        return 0;
    }

    if (strcmp(operator_node->text, "=") == 0) {
        condition->type = ID_INDEX_CONDITION_EQ;
        return 1;
    }
    if (strcmp(operator_node->text, ">") == 0) {
        condition->type = ID_INDEX_CONDITION_GT;
        return 1;
    }
    if (strcmp(operator_node->text, ">=") == 0) {
        condition->type = ID_INDEX_CONDITION_GE;
        return 1;
    }
    if (strcmp(operator_node->text, "<") == 0) {
        condition->type = ID_INDEX_CONDITION_LT;
        return 1;
    }
    if (strcmp(operator_node->text, "<=") == 0) {
        condition->type = ID_INDEX_CONDITION_LE;
        return 1;
    }
 ...
 */
static int read_row_by_offset(FILE *file, const TableMeta *meta, long offset, unsigned char *row, Status *status) {
    if (fseek(file, offset, SEEK_SET) != 0) {
        snprintf(status->message, sizeof(status->message), "Execution error: failed to seek row offset %ld", offset);
        return 0;
    }
    if (fread(row, 1, (size_t)meta->row_size, file) != (size_t)meta->row_size) {
...
static int print_indexed_row_at_offset(FILE *file, const TableMeta *meta, long offset, const int *selected_indexes, int selected_count, int *rows_selected, Status *status) {
    unsigned char row[MAX_ROW_SIZE]; /* offset으로 찾은 row를 읽을 버퍼 */

    if (!read_row_by_offset(file, meta, offset, row, status)) {
        return 0;
    }

    print_selected_row(meta, row, selected_indexes, selected_count);
    (*rows_selected)++;
    return 1;
}
...
static int select_by_id_index(const ExecutionContext *context, int target_id, const int *selected_indexes, int selected_count, Status *status) {
    ...
    int rows_selected = 0;       /* 실제로 출력한 row 수 */
    ...
    if (!print_indexed_row_at_offset(file, &context->meta, offset, selected_indexes, selected_count, &rows_selected, status)) {
        fclose(file);
        return 0;
    }
...
static int select_by_id_index_ge(const ExecutionContext *context, int lower_bound, const int *selected_indexes, int selected_count, Status *status) {
    FILE *file;
    BPTreeNode *leaf;
    int index;
    int rows_selected = 0;

    if (!bptree_find_lower_bound(&context->id_index, lower_bound, &leaf, &index)) {
        printf("0 rows selected.\n");
        return 1;
    }
    ...
    while (leaf != NULL) {
        for (; index < leaf->key_count; index++) {
            if (!print_indexed_row_at_offset(file, &context->meta, leaf->offsets[index], selected_indexes, selected_count, &rows_selected, status)) {
                fclose(file);
                return 0;
            }
        }
...
static int select_by_id_index_gt(const ExecutionContext *context, int lower_exclusive, const int *selected_indexes, int selected_count, Status *status) {
    FILE *file;
    BPTreeNode *leaf;
    int index;
    int rows_selected = 0;

    if (!bptree_find_lower_bound(&context->id_index, lower_exclusive, &leaf, &index)) {
        printf("0 rows selected.\n");
        return 1;
    }

    while (leaf != NULL && index < leaf->key_count && leaf->keys[index] <= lower_exclusive) {
        index++;
...
static int select_by_id_index_le(const ExecutionContext *context, int upper_bound, const int *selected_indexes, int selected_count, int inclusive, Status *status) {
    FILE *file;
    BPTreeNode *leaf;
    int rows_selected = 0;
    int stop = 0;

    leaf = bptree_leftmost_leaf(&context->id_index);
    if (leaf == NULL) {
        printf("0 rows selected.\n");
        return 1;
    }
    ...
    while (leaf != NULL && !stop) {
        ...
        for (index = 0; index < leaf->key_count; index++) {
            ...
            int in_range = inclusive ? (key <= upper_bound) : (key < upper_bound);
...
static int select_by_id_index_between(const ExecutionContext *context, int lower_bound, int upper_bound, const int *selected_indexes, int selected_count, Status *status) {
    FILE *file;
    BPTreeNode *leaf;
    int index;
    int rows_selected = 0;
    ...
    while (leaf != NULL && !stop) {
        for (; index < leaf->key_count; index++) {
            if (leaf->keys[index] > upper_bound) {
                stop = 1;
                break;
            }
            if (!print_indexed_row_at_offset(file, &context->meta, leaf->offsets[index], selected_indexes, selected_count, &rows_selected, status)) {
...
int execute_select(ExecutionContext *context, ASTNode *root, Status *status) {
    ...
    IdIndexCondition index_condition;  /* id 인덱스로 처리 가능한 WHERE 요약 */
    ...
    if (extract_id_index_condition(where_node, &index_condition)) {
        context->last_execution_path = EXECUTION_PATH_INDEXED;
        switch (index_condition.type) {
        case ID_INDEX_CONDITION_EQ:
            return select_by_id_index(context, index_condition.value, selected_indexes, selected_count, status);
        case ID_INDEX_CONDITION_GT:
            return select_by_id_index_gt(context, index_condition.value, selected_indexes, selected_count, status);
        case ID_INDEX_CONDITION_GE:
            return select_by_id_index_ge(context, index_condition.value, selected_indexes, selected_count, status);
        case ID_INDEX_CONDITION_LT:
            return select_by_id_index_le(context, index_condition.value, selected_indexes, selected_count, 0, status);
        case ID_INDEX_CONDITION_LE:
            return select_by_id_index_le(context, index_condition.value, selected_indexes, selected_count, 1, status);
        case ID_INDEX_CONDITION_BETWEEN:
            return select_by_id_index_between(context, index_condition.lower, index_condition.upper, selected_indexes, selected_count, status);
        default:
            break;
        }
    }
